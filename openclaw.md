...
- 已获取 AICentOS API Key（[控制台获取](https://aicentos.com/console/token)）
...
::: warning 重要
- 请将 `sk-你的FishXCode-Token` 替换为你在 [AICentOS 控制台](https://aicentos.com/console/token) 获取的实际 Token
...
::: tip 提示
**OpenAI 协议需要带 `/v1`**，即 `https://aicentos.com/v1`。这是因为两种 SDK 的路径拼接逻辑不同。
:::
...
| --- | --- | --- | --- |
| `baseUrl` | API 代理地址 | `https://aicentos.com` | `https://aicentos.com/v1` |
